...
class GraphQlHelper
{
    ...
    public function extractRelationsForEntity(array $match, array $allEntities): array
    {
        ...
        foreach ($lines as $line) {
            ...
            $relatedEntityName = mb_trim($rawType, '[]!'); // Remove list and non-null markers
    ...
    {
        $graphQlType = GraphQlType::tryFrom(mb_trim($rawType, '!'));
        if ($graphQlType === null) {

            // Try to see if it's a registered enum
            $typeName = mb_trim($rawType, '[]!');
    ...
    public function extractMinMax(array $directives): array
    {
        $min = $max = null;
        foreach ($directives as $directive) {
            if (preg_match('/@min\((.*?)\)/', $directive, $match)) {
                $min = (int) $match[1];
            }
            if (preg_match('/@max\((.*?)\)/', $directive, $match)) {
                $max = (int) $match[1];
            }
        }
    ...
    public function parseKeys(array $directives): array
    {
        ...
        foreach ($directives as $directive) {
            if (preg_match('/@foreignKey\((.*?)\)/', $directive, $match)) {
                $foreignKey = mb_trim($match[1], '"\'');
            }
            if (preg_match('/@localKey\((.*?)\)/', $directive, $match)) {
                $localKey = mb_trim($match[1], '"\'');
            }
            if (preg_match('/@relatedForeignKey\((.*?)\)/', $directive, $match)) {
                $relatedForeignKey = mb_trim($match[1], '"\'');
            }
        }
    ...
    public function parseRelationName(array $directives, string $defaultName): string
    {
        ...
        foreach ($directives as $directive) {
            if (preg_match('/@name\((.*?)\)/', $directive, $match)) {
                return mb_trim($match[1], '"\'');
            }
        }
